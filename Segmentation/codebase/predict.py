...
def evaluate( model=None , inp_images=None , annotations=None, inp_images_dir=None ,annotations_dir=None , checkpoints_path=None ):
    ...
    # tn = np.sum(n_pixels) - (tp + fp + fn)
    # print("TP: ", np.mean(tp), "TN: ", np.mean(tn), "FP: ",np.mean(fp), "FN: ", np.mean(fn))
    # iou = np.mean(tp) / ( np.mean(tp) + np.mean(fp) + np.mean(fn) + 0.000000000001 )
    # Accuracy = (np.mean(tp) + np.mean(tn)) / (np.mean(tp) + np.mean(tn) + np.mean(fp) + np.mean(fn))
    # ppv = np.mean(tp)/(np.mean(tp)+np.mean(fp))
